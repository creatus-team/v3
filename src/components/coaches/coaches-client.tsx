...
export function CoachesClient({ initialCoaches }: CoachesClientProps) {
    ...
    switch (slotFilter) {
      ...
      case 'endingSoon':
        ...
        const dDay = Math.ceil(
          (new Date(slot.currentSession.end_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
        );
  ...
  const calcDDay = (endDate: string) => {
    const d = Math.ceil((new Date(endDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
              ...
              <div className="space-y-3">
                {filteredSlots.map((slot) => {
                  ...
                  const isEndingSoon = session?.end_date && 
                    Math.ceil((new Date(session.end_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)) <= 7;
